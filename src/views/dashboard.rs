...
fn draw_nav(f:&mut Frame, app:&mut App, area:Rect){
    ...
    for (i,id) in items.iter().enumerate(){ let y=area.y+2+i as u16; let target=ClickTarget::SidebarGroup((*id).into()); app.mouse.register(Rect{x:area.x+1,y,width:area.width-2,height:1}, target.clone()); rows.push(ListItem::new(*id).style(button::row_style(app,&target,false))); }
...
fn draw_hosts(f:&mut Frame, app:&mut App, area:Rect){
    ...
    let add_target=ClickTarget::ModalButton("add-host".into());
    let add=Rect{x:area.x+area.width.saturating_sub(16),y:area.y,width:14,height:1}; app.mouse.register(add, add_target.clone());
...
fn draw_details(f:&mut Frame, app:&mut App, area:Rect){
    ...
    let mut spans=Vec::new(); for (i,(label,target)) in buttons.iter().enumerate(){ let x=area.x+2+(i as u16%2)*14; let y=area.y+8+(i as u16/2)*2; app.mouse.register(Rect{x,y,width:13,height:1}, target.clone()); let kind=if *label=="Connect"{ButtonKind::Primary}else{ButtonKind::Secondary}; spans.push(button::label(app,label,target,kind)); if i==1 { lines.push(Line::from(spans.clone())); spans.clear(); } } if !spans.is_empty(){lines.push(Line::from(spans));}
