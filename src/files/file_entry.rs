...
impl FileEntry {
    pub fn icon(&self, nerd: bool) -> &'static str {
        if nerd {
            match self.kind {
                FileKind::Directory => "󰉋",
                FileKind::Symlink => "",
                FileKind::Executable => "󰡱",
                FileKind::Archive => "󰿺",
                FileKind::Image => "󰋩",
                _ => "󰈙",
            }
...
pub fn parse_ls_line(line: &str) -> Option<FileEntry> {
...
mod tests {
    ...
    #[test]
    fn parses_ls_entry() {
        let e = parse_ls_line("-rw-r--r-- 1 user group 2048 May 16 09:00 package.json").unwrap();
        assert_eq!(e.name, "package.json");
        assert_eq!(e.size, 2048);
        assert_eq!(e.kind, FileKind::File);
    }

    #[test]
    fn parses_symlink_entry_name_without_target() {
        let e = parse_ls_line("lrwxrwxrwx 1 user group 9 May 16 09:00 link -> /tmp/real").unwrap();
